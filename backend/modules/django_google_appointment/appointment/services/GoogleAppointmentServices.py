...
class GoogleAppointmentService:
    def __init__(self, access_token=None, credential_file_path=None):
        ...
        try:
            ...
            self.google_appointment_service = discovery.build(
    ...
    def appointment_list(
        ...
        try:
            events_list = (
                self.google_appointment_service.events()
                .list(
                    calendarId="primary",
    ...
    def single_appointment(self, eventId):
        try:
            event = (
                self.google_appointment_service.events()
                .get(calendarId="primary", eventId=eventId)
                .execute()
            )
            return event
        except Exception:
    ...
    def create_appointment(self, payload):
        try:
            event = (
                self.google_appointment_service.events()
                .insert(calendarId="primary", body=payload, conferenceDataVersion=1)
                .execute()
            )
            return event
        except Exception:
    ...
    def delete_appointment(self, eventId):
        try:
            self.google_appointment_service.events().delete(
